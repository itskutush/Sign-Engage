...
class GreetingsListViewController: UIViewController , UICollectionViewDataSource,UICollectionViewDelegateFlowLayout{
    
    // Accessing the shared instance of the video data model.
    let videoDataModel = GreetingsDataModel.shared
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "GreetingsVideoNameCell", for: indexPath) as! GreetingsVideoNameCell
...
//till this much is collection view flow layout 2
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier == "ShowGreetingsVideo"{
            if let indexPath = collectionView.indexPathsForSelectedItems?.first{
                let selectedVideo = videoDataModel.videos[indexPath.row]
                
                if let videoVC = segue.destination as? GreetingsVideoViewController{
                    videoVC.GreetingsselectedVideo = selectedVideo
                }
