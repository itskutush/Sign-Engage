...
class NumbersListViewController: UIViewController , UICollectionViewDataSource,UICollectionViewDelegateFlowLayout{
    
    // Accessing the shared instance of the video data model.
    let videoDataModel = NumbersDataModel.shared
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "NumbersVideoNameCell", for: indexPath) as! NumbersVideoNameCell
...
//till this much is collection view flow layout 2
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier == "ShowNumbersVideo"{
            if let indexPath = collectionView.indexPathsForSelectedItems?.first{
                let selectedVideo = videoDataModel.videos[indexPath.row]
                
                if let videoVC = segue.destination as? NumbersVideoViewController{
                    videoVC.NumbersselectedVideo = selectedVideo
                }
