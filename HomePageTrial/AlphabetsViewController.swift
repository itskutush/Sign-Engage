...
class AlphabetsViewController: UIViewController {
    ...
    let alphabets = ["A", "C", "E", "F", "G", "I", "L", "M", "N", "O", "T", "U", "V", "X", "Y", "Z"]
    ...
    @objc func alphabetButtonTapped(_ sender: UIButton) {
        ...
        if let title = sender.currentTitle,
           let video = AlphabetsDataModel.shared.videos.first(where: { $0.title == title }) {
            
            // Navigate to LearnViewController and pass the selected video
            let storyboard = UIStoryboard(name: "Main", bundle: nil)
            if let selectedAlphabet = sender.currentTitle {
                let storyboard = UIStoryboard(name: "Main", bundle: nil)
                if let performVC = storyboard.instantiateViewController(withIdentifier: "PerformViewController") as? PerformViewController {
                    performVC.selectedAlphabet = selectedAlphabet
                    navigationController?.pushViewController(performVC, animated: true)
                }
            }
        }
    }
