...
class AlphabetsQuizViewController: UIViewController {
    ...
    func playVideo(for question: AlphabetsQuizDataModel) {
    ...
    func displayCurrentQuestion() {
        let question = AlphabetsquizQuestions[currentQuestionIndex]
        
        // Update the question number label and progress view
        questionNumberLabel.text = "\(currentQuestionIndex + 1) / \(AlphabetsquizQuestions.count)"
        progressView.progress = Float(currentQuestionIndex + 1) / Float(AlphabetsquizQuestions.count)
    ...
    @IBAction func nextButtonTapped(_ sender: UIButton) {
        ...
        if currentQuestionIndex < AlphabetsquizQuestions.count {
            displayCurrentQuestion()
            [optionButton1, optionButton2, optionButton3, optionButton4].forEach { $0.isEnabled = true }
        } else {
            performSegue(withIdentifier: "showAlphabetsQuizCompletionScreen", sender: nil)
        }
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "showAlphabetsQuizCompletionScreen" {
            let destinationVC = segue.destination as! AlphabetsQuizCompletionViewController
            destinationVC.AlphabetsQuizTotalScore = correctAnswersCount
        }
    }
}
